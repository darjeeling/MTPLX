...
Point OpenCode or Pi at that port using the normal MTPLX integration. Let coding tasks stop naturally. Preserve both the client transcript and the generated project. To keep a record of each request for a local reproduction, set `MTPLX_REQUEST_CAPTURE_DIR` before starting the server. By default a record holds the sampler settings, the seed, token counts and SHA-256 digests, and no prompt text, answer text or token IDs, so it is safe to attach to an issue. An exact replay needs the IDs themselves: add `MTPLX_REQUEST_CAPTURE_INCLUDE_PROMPT_TOKENS=1` and `MTPLX_REQUEST_CAPTURE_INCLUDE_COMPLETION_TOKENS=1`. Those two keep the whole sequences (set `MTPLX_REQUEST_CAPTURE_PROMPT_TOKEN_LIMIT` or `MTPLX_REQUEST_CAPTURE_COMPLETION_TOKEN_LIMIT` to bound them), and a record made that way contains your prompt, so keep it local unless you intend to share that content. `MTPLX_REQUEST_CAPTURE_INCLUDE_RESPONSE_TEXT=1` adds a clipped head and tail of the answer. The capture folder is a ring: `MTPLX_REQUEST_CAPTURE_KEEP` (default 200) is how many `req-*.json` files stay in place; older ones move to a `pruned/` subfolder and are never deleted.
