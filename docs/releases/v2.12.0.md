...
- **It runs on 16 GB Macs.** The memory planner used to refuse Bonsai on a 16 GB Mac. It now admits it with an 8,192-token window (see Memory below). Measured by limiting a 128 GB M5 Max to each class's memory budget, with the draft head and the vision tower loaded:
